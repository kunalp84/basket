...
const createActionTeacherEnquiryView = (props, values) => {
 ...
 var startPoint, sizePoint,textFilter
        ...
        if(props.customFilter && props.customFilter.length>3)
        {
            ...
            if(props.customFilter !== sessionStorage.getItem("filterForBatch") )
              {
                console.log("Reset as change in text filter")
                props.dispatch({type: 'RESET_ENQUIRY_LIST_PAGE', data:[]});

                startPoint=0
                sizePoint=10  
                sessionStorage.setItem("filterForBatch",props.customFilter)    

              
              }

              textFilter= sessionStorage.getItem("filterForBatch")
              ...
              // return (dispatch) => {
               ...
               headers: {
                   'Content-Type': 'application/json'
                     ...
                     .then((res) =>{
                       console.log(`http://0.0.0.0:9200/requirement/batch/_search?q=*${textFilter}*&from=${startPoint}&size=${sizePoint}`);
                       var dataRecieved =[]
                       ...
                       if(startPoint===0)
                       {
                         props.dispatch({type: 'LOAD_ENQUIRY', data:{nextElements:dataRecieved,totalRecords:totalRecords}});
                          dataRecieved =[]
                       }
                       else
                       {
                         props.dispatch({type: 'LOAD_MORE', data:{nextElements:dataRecieved, pageToBeFetched:props.pageToBeFetched, start:startPoint, size:sizePoint,totalRecords:totalRecords}  }) ;
                          dataRecieved =[]
                       }
